...
import { Beer, User, Mail, Lock, AlertCircle, Loader2, Eye, EyeOff, ArrowLeft } from 'lucide-react';
...
const Register = () => {
  ...
  const onSubmit = async (e) => {
    ...
    try {
      ...
      navigate(getDashboardPath(registeredUser), { replace: true });
    } catch {
      // registration failure will be handled by auth state error display
    }
  };
      ...
      <div className="w-full max-w-md glass-panel p-8 rounded-2xl shadow-2xl relative z-10">
        ...
        <div className="flex flex-col items-center gap-2 mb-8 text-center">
          ...
          <p className="text-slate-400 text-sm">
            Create your customer account for bookings and shopping
          </p>
        </div>
        ...
        {isError && (
          <div className="mb-6 p-4 rounded-xl bg-red-500/10 border border-red-500/20 text-red-400 text-sm flex items-center gap-3">
            ...
            <span>{message || 'Registration failed'}</span>
          </div>
        )}
